...
export function AppSidebar({ mode }: { mode: "owner" | "admin" }) {
  ...
  const googleLink =
    mode === "owner" ? (
      <a
        ...
        target="_blank"
        rel="noreferrer"
        className="flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-semibold text-white/50 transition hover:bg-white/10 hover:text-white"
      >
  ...
  const content = (
    <>
      {mode === "owner" ? (
        <Link
          href="/onboarding"
          className="mx-4 mt-5 flex h-11 items-center justify-center gap-2 rounded-xl bg-[#2463f3] text-sm font-extrabold shadow-[0_8px_20px_rgba(36,99,243,0.22)] transition hover:bg-[#3a73f5]"
        >
      ...
      <nav
        className="flex-1 space-y-1 overflow-y-auto px-3 py-7"
      ...
      >
        ...
        {accountNav.length ? (
          <>
            <p className="mb-3 mt-7 border-t border-white/10 px-3 pt-6 text-[10px] font-bold uppercase tracking-[0.2em] text-white/30">
              Account
            </p>
        ...
        {googleLink ? (
          <div className="mt-7 border-t border-white/10 pt-5">{googleLink}</div>
        ) : null}
      </nav>
    </>
  );
  ...
  return (
    <>
      ...
      <div className="sticky top-0 z-40 flex items-center justify-between border-b border-white/10 bg-[#0b1428] px-4 py-3 text-white lg:hidden">
...
function NavLink({
  ...
  return (
    <Link
      ...
      className={`relative flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-semibold transition ${active ? "bg-[#142653] text-white before:absolute before:inset-y-2 before:left-0 before:w-1 before:rounded-r-full before:bg-[#3c7bff]" : "text-white/50 hover:bg-white/10 hover:text-white"}`}
    >
...
function Brand({
  ...
  return (
    <div
      className={`flex items-center ${compact ? "" : "h-[76px] border-b border-white/10 px-5"}`}
    >
      <Link
