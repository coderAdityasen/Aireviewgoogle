...
export default async function DashboardPage() {
  ...
  return <div className="space-y-7">
    ...
    <div className="grid gap-6 xl:grid-cols-[1.35fr_0.65fr]">
      ...
      <Card><CardHeader><CardTitle>Quick start</CardTitle><p className="mt-1 text-sm font-medium text-muted-foreground">Test the customer experience before sharing it.</p></CardHeader><CardContent className="space-y-3"><div className="rounded-2xl bg-[#f5f7fb] p-4"><p className="text-[10px] font-extrabold uppercase tracking-[0.16em] text-muted-foreground">Active location</p><p className="mt-2 text-lg font-extrabold tracking-[-0.04em]">{firstBusiness.name}</p><Badge className="mt-2">{firstBusiness.is_active ? "Active" : "Disabled"}</Badge></div><Button asChild className="w-full"><Link href={`/r/${firstBusiness.slug}`}>Test customer flow</Link></Button><Button asChild variant="outline" className="w-full"><Link href="/dashboard/qr-posters">Download QR</Link></Button></CardContent></Card>
    </div>

    <div className="grid gap-6 xl:grid-cols-[1.15fr_0.85fr]">
      <Card><CardHeader className="flex-row items-center justify-between"><div><CardTitle>Locations</CardTitle><p className="mt-1 text-sm font-medium text-muted-foreground">Your active review destinations.</p></div><Button asChild size="sm"><Link href="/onboarding">Add location</Link></Button></CardHeader><CardContent className="space-y-2">{metrics.businesses.map((business) => <div key={business.id} className="flex items-center justify-between gap-3 rounded-2xl border border-border/70 px-4 py-3"><div className="min-w-0"><Link className="truncate text-sm font-extrabold hover:text-primary" href={`/dashboard/businesses/${business.id}`}>{business.name}</Link><p className="mt-1 text-xs font-medium text-muted-foreground">{business.is_active ? "Accepting customer feedback" : "Disabled"}</p></div><Badge>{business.is_active ? "Active" : "Disabled"}</Badge></div>)}</CardContent></Card>
