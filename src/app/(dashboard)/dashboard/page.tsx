...
export default async function DashboardPage() {
  ...
  if (!metrics.businesses.length || !firstBusiness) {
    return <EmptyState title="Your workspace is ready" description="Connect your first location to create a customer review flow and start receiving useful feedback." action={{ href: "/dashboard/businesses/new", label: "Set up a location" }} />;
  }
  ...
  return <div className="space-y-7">
    ...
    <div className="grid gap-6 xl:grid-cols-[1.15fr_0.85fr]">
      <Card><CardHeader className="flex-row items-center justify-between"><div><CardTitle>Locations</CardTitle><p className="mt-1 text-sm font-medium text-muted-foreground">Your active review destinations.</p></div><Button asChild size="sm"><Link href="/dashboard/businesses/new">Add location</Link></Button></CardHeader><CardContent className="space-y-2">{metrics.businesses.map((business) => <div key={business.id} className="flex items-center justify-between gap-3 rounded-2xl border border-border/70 bg-card px-4 py-3 transition-colors hover:border-primary/20 hover:bg-muted/30"><div className="min-w-0"><Link className="cursor-pointer truncate text-sm font-extrabold hover:text-primary" href={`/dashboard/businesses/${business.id}`}>{business.name}</Link><p className="mt-1 text-xs font-medium text-muted-foreground">{business.is_active ? "Accepting customer feedback" : "Disabled"}</p></div><Badge variant={business.is_active ? "success" : "warning"}>{business.is_active ? "Active" : "Disabled"}</Badge></div>)}</CardContent></Card>
