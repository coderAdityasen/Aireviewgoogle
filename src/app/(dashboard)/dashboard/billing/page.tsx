...
export default async function DashboardBillingPage() {
  ...
  return <div className="space-y-5 pb-8">
    ...
    <div className="grid gap-5 xl:grid-cols-[1.35fr_0.65fr]">
      <Card><CardHeader className="flex-row items-center justify-between gap-4"><div><p className="text-[10px] font-extrabold uppercase tracking-[0.18em] text-primary">Current subscription</p><CardTitle className="mt-2 text-2xl">{currentPlan?.name ?? "Choose a plan"}</CardTitle></div>{subscription ? <Badge>{subscription.status}</Badge> : null}</CardHeader><CardContent><div className="grid gap-5 sm:grid-cols-2"><Detail label="Price & interval" value={currentPlan ? (currentPlan.key === "starter" ? "Free 7-day trial" : currentPlan.contactSales ? "Custom pricing" : `₹${currentPlan.priceInr.toLocaleString("en-IN")} / month`) : "Not active"} /><Detail label="Access until" value={entitlements.trialActive && entitlements.trialEndsAt ? new Date(entitlements.trialEndsAt).toLocaleDateString() : subscription?.access_until ? new Date(subscription.access_until).toLocaleDateString() : "-"} /><Detail label="Billing cycle" value={currentPlan?.key === "starter" ? "Trial" : subscription?.current_period_end ? "Monthly renewal" : "-"} /><Detail label="Payment provider" value={currentPlan?.key === "starter" ? "No payment on trial" : "Razorpay Test Mode"} /></div><div className="mt-6 flex flex-wrap gap-2">{subscription && currentPlan ? <CancelSubscriptionButton scheduled={subscription.cancel_at_period_end} /> : null}<Button asChild><Link href="#plans">{subscription ? "Change plan" : "Choose a plan"}</Link></Button></div></CardContent></Card>
