...
export default async function DashboardBillingPage() {
  ...
  return <div className="space-y-5 pb-8">
    ...
    <Card className="overflow-hidden"><CardHeader className="flex-row items-center justify-between gap-4 border-b bg-white"><div className="flex items-center gap-3"><span className="grid h-11 w-11 place-items-center rounded-xl bg-primary text-white">▤</span><div><CardTitle className="text-base uppercase tracking-[0.08em]">Subscription & usage</CardTitle><p className="mt-1 text-sm font-medium text-muted-foreground">Your current billing period</p></div></div><div className="text-right"><p className="text-xs font-bold text-muted-foreground">Current plan</p><Badge className="mt-1">{currentPlan?.name ?? "No active plan"}</Badge></div></CardHeader><CardContent className="grid gap-5 p-5 sm:p-6"><Usage label="Business locations" value={entitlements.usage.businesses} limit={entitlements.plan.businesses} /><Usage label="Review requests" value={entitlements.usage.reviewRequests} limit={entitlements.plan.reviewRequests} /><Usage label="Regenerations" value={entitlements.usage.aiGenerations} limit={entitlements.plan.aiGenerations} /></CardContent></Card>

    <div className="grid gap-5 xl:grid-cols-[1.35fr_0.65fr]">
      <Card><CardHeader className="flex-row items-center justify-between gap-4"><div><p className="text-[10px] font-extrabold uppercase tracking-[0.18em] text-primary">Current subscription</p><CardTitle className="mt-2 text-2xl">{currentPlan?.name ?? "Choose a plan"}</CardTitle></div>{subscription ? <Badge>{subscription.status}</Badge> : null}</CardHeader><CardContent><div className="grid gap-5 sm:grid-cols-2"><Detail label="Price & interval" value={currentPlan ? (currentPlan.key === "starter" ? "Free 7-day trial" : currentPlan.contactSales ? "Custom pricing" : `Growth one-time (from ₹${currentPlan.priceInr.toLocaleString("en-IN")})`) : "Not active"} /><Detail label="Access until" value={entitlements.trialActive && entitlements.trialEndsAt ? new Date(entitlements.trialEndsAt).toLocaleDateString() : subscription?.access_until ? new Date(subscription.access_until).toLocaleDateString() : "-"} /><Detail label="Billing cycle" value={currentPlan?.key === "starter" ? "Trial" : subscription?.current_period_end ? "One-time access (no auto-renewal)" : "-"} /><Detail label="Payment provider" value={currentPlan?.key === "starter" ? "No payment on trial" : "Razorpay"} /></div><div className="mt-6 flex flex-wrap gap-2">{subscription && currentPlan ? <CancelSubscriptionButton scheduled={subscription.cancel_at_period_end} /> : null}<Button asChild><Link href="#plans">{subscription ? "Change plan" : "Choose a plan"}</Link></Button></div></CardContent></Card>
  ...
  return (
    <div>
      ...
      <div className="mt-2 h-2.5 rounded-full bg-muted">
        <div className="h-2.5 rounded-full bg-primary transition-[width] duration-500 motion-reduce:transition-none" style={{ width: `${width}%` }} />
      </div>
    </div>
  );
}
