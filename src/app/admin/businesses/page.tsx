              ...
              <Td>
                <details>
                  ...
                  <form action={updateBusinessAdminAction} className="mt-3 grid gap-2">
                    ...
                    <div className="rounded-xl border border-primary/15 bg-primary/[0.03] p-3"><p className="text-xs font-extrabold text-foreground">Customer options by rating</p><p className="mt-1 text-[11px] font-medium leading-4 text-muted-foreground">One option per line. These appear after the customer chooses a rating.</p><div className="mt-3 grid gap-2 sm:grid-cols-2">{([1, 2, 3, 4, 5] as const).map((rating) => <label key={rating} className="text-xs font-medium text-muted-foreground">{rating}-star options<textarea name={`ratingTags${rating}`} defaultValue={ratingTagText(business.experience_tags, rating)} placeholder={rating >= 4 ? "Friendly service\nClear communication" : "What could be improved"} className="mt-1 min-h-16 w-full rounded-md border bg-card p-2 text-sm text-foreground" /></label>)}</div></div>
                    <label className="text-xs font-medium text-muted-foreground">Low-rating support message<textarea name="lowRatingSupportMessage" defaultValue={business.low_rating_support_message ?? ""} className="mt-1 min-h-16 w-full rounded-md border bg-card p-2 text-sm text-foreground" /></label>
                    <label className="text-xs font-medium text-muted-foreground">Private follow-up fields<input name="contactFields" defaultValue={Array.isArray(business.contact_fields) ? business.contact_fields.join(",") : "name,email"} placeholder="name,email" className="mt-1 h-9 w-full rounded-md border bg-card px-2 text-sm text-foreground" /></label>
