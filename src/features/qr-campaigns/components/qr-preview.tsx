    ...
    <div className="grid gap-6 lg:grid-cols-[360px_1fr]">
      <div className="rounded-md border bg-card p-5">
        <div className="relative mx-auto aspect-square w-full max-w-80 bg-white p-3">
          {png ? <img src={png} alt={`${businessName} QR code`} className="h-full w-full" /> : null}
          {logoUrl && logoOverlay ? (
            <img
              src={logoUrl}
              alt=""
        ...
        <div className="mt-4 space-y-2 text-sm">
          ...
          {logoUrl ? (
            <label className="flex items-center gap-2">
              <input type="checkbox" checked={logoOverlay} onChange={(event) => setLogoOverlay(event.target.checked)} />
              Add small logo overlay with high error correction
            </label>
          ) : null}
        </div>
        <div className="mt-4 flex flex-wrap gap-2">
          <Button type="button" variant="outline" onClick={() => navigator.clipboard.writeText(url).then(() => toast.success("URL copied."))}>
